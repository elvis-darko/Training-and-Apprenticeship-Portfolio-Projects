...
def welcome_page():
    ...
    st.write("<h2 style='font-size: 24px;'> Text Classification Models developped to ascertain public perception of covid vaccines </h2>",unsafe_allow_html=True)
...
def prediction_page():
  with st.form(key='tweet', clear_on_submit=True):
      ...
      #Create alternative tweets to allow users to pick easily
      alt_text=st.selectbox( "Please choose any of the sample tweets",
        ('Vaccines have been good so far', 
        'Had a bad experience with the vaccine', 
        'Covid is human made. The vaccines are deadly', 
        'Unqualified people administered  vaccine',
        'Vaccine is dangerous to women', 
        'Vaccine can kill people with anaemia', 
        'Vaccine protects us from the deadly virus'))
   
      #Choose model to use for prediction
      models={
          'Bert':'elvis-d/elvis_bert_base',
